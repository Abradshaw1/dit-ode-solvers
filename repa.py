...
def load_encoder(encoder_type="dinov2", model_size="s", device="cuda"):
    ...
    if "dinov2" in encoder_type:
        ...
        if encoder_type == "clip":
            model_names = {"s": "ViT-B-16", "b": "ViT-B-16", "l": "ViT-L-14"}
            pretrained = "openai"
        else:
            model_names = {"s": "ViT-B-16-SigLIP", "b": "ViT-B-16-SigLIP", "l": "ViT-L-16-SigLIP-256"}
            pretrained = "webli"
        model_name = model_names[model_size]
        model, _, _ = open_clip.create_model_and_transforms(model_name, pretrained=pretrained)
        if hasattr(model.visual, 'trunk'):
            encoder = model.visual.trunk
            embed_dim = encoder.embed_dim
        else:
            encoder = model.visual
            embed_dim = encoder.conv1.out_channels
        encoder = encoder.to(device)
        encoder.eval()
        return encoder, embed_dim
    else:
...
def preprocess_for_encoder(x, encoder_type="dinov2", target_size=224):
    ...
    # Normalize with appropriate stats per encoder
    if encoder_type == "clip":
        x = Normalize(CLIP_MEAN, CLIP_STD)(x)
    elif encoder_type == "siglip":
        x = Normalize(SIGLIP_MEAN, SIGLIP_STD)(x)
    else:
...
def extract_encoder_features(encoder, images, encoder_type="dinov2"):
    ...
    if "dinov2" in encoder_type:
        ...
        # SigLIP uses timm trunk: forward_features returns (B, 1+N, D)
        features = encoder.forward_features(images)
        features = features[:, encoder.num_prefix_tokens:]
    elif encoder_type == "clip":
        # open_clip native VisionTransformer: manual forward to get patch tokens
        v = encoder
        x = v.conv1(images)
